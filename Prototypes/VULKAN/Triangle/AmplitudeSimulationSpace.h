...
class AmplitudeSimulationSpace
{
...
public:
	 ...
	 */
	std::pair<glm::vec3, glm::vec3> waterSurface(glm::vec2 pos) const;
	 ...
	 * this box.
	 */
	float amplitude(glm::vec4 pos4) const;
	 ...
	 */
	std::vector<glm::vec4> trajectory(glm::vec4 pos4, float length) const;
...
public:
	 ...
	 * to the boundary.
	 */
	glm::vec4 boundaryReflection(glm::vec4 pos4) const;
	...
	float DomainInterpolation(glm::vec4 pos4) const;

public:
	float idxToPos(int idx, int dim) const;
	glm::vec4 idxToPos(Idx idx) const;

	float posToGrid(float pos, int dim) const;
	glm::vec4 posToGrid(glm::vec4 pos4) const;

	int posToIdx(float pos, int dim) const;
	Idx posToIdx(glm::vec4 pos4) const;

	glm::vec2 nodePosition(int ix, int iy) const;

	float waveLength(int izeta) const;
	float waveNumber(int izeta) const;

	float dispersionRelation(float k) const;
	float dispersionRelation(glm::vec4 pos4) const;
...
public:
	AmplitudeGrid m_amplitude, m_newAmplitude;
	WaveSpectrum m_spectrum;

	std::vector<WaveProfileBuffer> m_profileBuffers;
