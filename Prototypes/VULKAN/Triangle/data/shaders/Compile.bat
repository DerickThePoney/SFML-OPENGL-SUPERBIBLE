%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberNoOpacity.vert -o data/shaders/UberNoOpacity.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberNoOpacity.frag -o data/shaders/UberNoOpacity.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberOpacity.frag -o data/shaders/UberOpacity.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberWater.vert -o data/shaders/UberWater.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberWater.tesc -o data/shaders/UberWater.tesc.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberWater.tese -o data/shaders/UberWater.tese.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UberWater.frag -o data/shaders/UberWater.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/shadowmappingdirectional.vert -o data/shaders/shadowmappingdirectional.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/shadowmappingdirectional.frag -o data/shaders/shadowmappingdirectional.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UI.vert -o data/shaders/UI.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/UI.frag -o data/shaders/UI.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/NormalsFragment.frag -o data/shaders/NormalsFragment.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/NormalsGeometry.geom -o data/shaders/NormalsGeometry.geom.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/WireframeFragment.frag -o data/shaders/WireframeFragment.frag.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/NormalsVertex.vert -o data/shaders/NormalsVertex.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/NormalsTessControl.tesc -o data/shaders/NormalsTessControl.tesc.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/NormalsTessEval.tese -o data/shaders/NormalsTessEval.tese.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/FullscreenTriangle.vert -o data/shaders/FullscreenTriangle.vert.spv
%VULKAN_SDK%\Bin32\glslangValidator.exe -Os -V data/shaders/FullscreenTriangle.frag -o data/shaders/FullscreenTriangle.frag.spv
pause